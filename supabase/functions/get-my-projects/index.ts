...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Start building the query for unit_projects
    let query = supabaseClient.from("unit_projects").select("*");

    // Check if the user is an admin
    const isAdmin = ADMIN_EMAILS.includes(user_email.toLowerCase());

    if (isAdmin) {
      console.log(`User ${user_email} is an admin. Fetching all projects.`);
      // Admin query remains selecting all projects
    } else {
      ...
      if (!userUnit) {
         console.warn(`Unit not found for user ${user_email}. Returning empty projects.`);
      ...
      console.log(`User ${user_email} belongs to unit: ${userUnit}. Filtering projects...`);
    ...
    console.log(`Successfully fetched ${data?.length ?? 0} projects for ${user_email} (Admin: ${isAdmin})`);
